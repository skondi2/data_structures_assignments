...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1Copy.begin(), word1Copy.end(), word1Copy.begin(), ::toupper);
    std::transform(word2Copy.begin(), word2Copy.end(), word2Copy.begin(), ::toupper);

    auto iter1 = dict.find(word1Copy);
    auto iter2 = dict.find(word2Copy);

    // false if either isn't in dictionary
    if (dict.find(word1Copy) == dict.end() || dict.find(word2Copy) == dict.end()) {
      return false;
    }

    std::vector<string> sounds1 = dict.at(word1Copy);
    std::vector<string> sounds2 = dict.at(word2Copy);

    // false is they don't have same number of sounds
