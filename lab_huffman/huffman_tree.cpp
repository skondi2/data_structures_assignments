                            ...
                            queue<TreeNode*>& mergeQueue)
{
  ...
  TreeNode* smallestSingleQ = singleQueue.front();
  TreeNode* smallestMergeQ = mergeQueue.front();

    TreeNode* smallest = smallestSingleQ;
    if (smallestSingleQ->freq.getFrequency() > smallestMergeQ->freq.getFrequency()) {
      smallest = smallestMergeQ;
      mergeQueue.pop();
    } else {
      singleQueue.pop();
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned i = 0; i < frequencies.size(); i++) {
      TreeNode* newNode = new TreeNode(frequencies.at(i));
    ...
    while (singleQueue.size() + mergeQueue.size() > 1 ) {
      ...
      TreeNode* internalNode = new TreeNode(firstSmallest->freq.getFrequency() + secondSmallest->freq.getFrequency());
      internalNode->left = firstSmallest;
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits() && current != NULL) {
      if (!bfile.getNextBit()) {
