...
template <typename T> T sum(stack<T>& s) {
  ...
  T temp = s.top();
  s.pop();
  T stackSum = sum(s) + temp;
  s.push(temp);
  return stackSum;

    // Your code here
    //return T(sum); // stub return value (0 for primitive types). Change this!
...
bool verifySame(stack<T>& s, queue<T>& q)
{
    ...
    T queueElement; // rename me
    T stackElement; // rename :)
    ...
    stackElement = s.top();
    s.pop();
    // check if same with changed stack
    retval = verifySame(s, q);
    queueElement = q.front();
    // remove the element from top of que
    q.pop();
    q.push(queueElement); // add to the bottom
    s.push(stackElement);
    return (stackElement == queueElement && retval);
