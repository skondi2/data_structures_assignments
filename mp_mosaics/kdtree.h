...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* buildTree(vector<Point<Dim>>& pointsVec, int leftBound, int rightBound, int dimension);

    int quickSelect(vector<Point<Dim>>& pointsVec, int l, int r, int k, int dim);

    int partition(vector<Point<Dim>>& pointsVec, int l, int r, int pivotIndex, int dim);

    double distanceNotRooted(Point<Dim> p1, Point<Dim> p2) const;

    Point<Dim> recurseDownTree(KDTreeNode* subroot, Point<Dim> target, int splittingDim) const;

    void copyhelper(KDTreeNode* root, KDTreeNode* otherRoot);
